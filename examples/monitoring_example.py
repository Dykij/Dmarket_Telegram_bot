...
async def custom_health_check() -> Dict[str, Any]:
    ...
    is_healthy = (time.time() % 30) > 5  # Healthy most of the time, unhealthy for 5 seconds every 30 seconds
...
async def main() -> None:
    ...
    try:
        ...
        health_check.register_check(
            "rabbitmq", 
            lambda: check_rabbitmq_connection(rabbitmq_connector)
        )
        health_check.register_check(
            "dmarket_api", 
            lambda: check_dmarket_api(dmarket_client)
        )
        health_check.register_check(
            "custom", 
            custom_health_check
