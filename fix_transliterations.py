...
def fix_file(file_path: Path, dry_run: bool = False) -> Tuple[int, str]:
    ...
    Args:
        ...
        dry_run: Если True, то файл не будет изменен
    ...
    replacements = 0
    for transliteration, correct in REPLACEMENTS.items():
        replacements += content.count(transliteration)
...
def process_files(
    directory: Path, extensions: List[str] = None, exclude: List[str] = None, dry_run: bool = False
) -> int:
    ...
    Args:
        ...
        dry_run: Если True, то файлы не будут изменены
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for file in files:
            ...
            if file_suffix in extensions:
                ...
                replacements, _ = fix_file(file_path, dry_run)
                if replacements > 0:
                    logger.info(f"Исправлено {replacements} транслитераций в {file_path}")
                    fixed_files += 1
...
def main():
    ...
    logger.info(f"Начинаем исправление транслитераций в директории: {project_root}")
    ...
    if additional_fixed > 0:
        logger.info(f"Дополнительно исправлено файлов: {additional_fixed}")
