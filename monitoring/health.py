...
class HealthCheck:
    ...
    async def check_component(self, name: str, check_func: HealthCheckFunc) -> ComponentHealth:
        ...
        try:
            ...
            component_health = ComponentHealth(
                name=name, status=status, details=details, last_check_time=time.time()
            )
            ...
            component_health = ComponentHealth(
                name=name, status=HealthStatus.UNHEALTHY, error=str(e), last_check_time=time.time()
            )
            self._results[name] = component_health
            return component_health
    ...
    async def check_health(self) -> HealthCheckResult:
        ...
        return HealthCheckResult(
            status=status, components=components, timestamp=self._last_check_time
        )
...
async def check_rabbitmq_connection(rabbitmq_connector) -> dict[str, Any]:
    ...
    try:
        ...
        if not rabbitmq_connector.is_connected():
            return {"status": "unhealthy", "details": {"connection": "closed"}}

        return {"status": "healthy", "details": {"connection": "open"}}
    except Exception as e:
        return {"status": "unhealthy", "details": {"error": str(e)}}
...
async def check_dmarket_api(dmarket_client) -> dict[str, Any]:
    ...
    try:
        ...
        if result is None:
            return {"status": "unhealthy", "details": {"api": "not responding"}}
